...
//		GroupBy(cartitem.FieldProductID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.CartItem.Query().
//		Select(cartitem.FieldProductID).
//		Scan(ctx, &v)
...
func (ciq *CartItemQuery) loadCart(ctx context.Context, query *CartQuery, nodes []*CartItem, init func(*CartItem), assign func(*CartItem, *Cart)) error {
	...
	for i := range nodes {
		if nodes[i].cart_item_cart == nil {
			continue
		}
		fk := *nodes[i].cart_item_cart
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cart_item_cart" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
