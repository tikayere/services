...
func (cq *CartQuery) loadCartItems(ctx context.Context, query *CartItemQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *CartItem)) error {
	...
	for _, n := range neighbors {
		fk := n.cart_cart_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "cart_cart_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "cart_cart_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
