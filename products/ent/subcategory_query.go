...
func (scq *SubCategoryQuery) QueryProducts() *ProductQuery {
...
func (scq *SubCategoryQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (scq *SubCategoryQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (scq *SubCategoryQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (scq *SubCategoryQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (scq *SubCategoryQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (scq *SubCategoryQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := scq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (scq *SubCategoryQuery) Clone() *SubCategoryQuery {
	...
	return &SubCategoryQuery{
		...
		withProducts: scq.withProducts.Clone(),
...
func (scq *SubCategoryQuery) WithProducts(opts ...func(*ProductQuery)) *SubCategoryQuery {
	query := (&ProductClient{config: scq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	scq.withProducts = query
	return scq
}
...
func (scq *SubCategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubCategory, error) {
	...
	if query := scq.withProducts; query != nil {
		if err := scq.loadProducts(ctx, query, nodes,
			func(n *SubCategory) { n.Edges.Products = []*Product{} },
			func(n *SubCategory, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (scq *SubCategoryQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*SubCategory, init func(*SubCategory), assign func(*SubCategory, *Category)) error {
	...
	for i := range nodes {
		if nodes[i].sub_category_category == nil {
			continue
		}
		fk := *nodes[i].sub_category_category
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sub_category_category" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (scq *SubCategoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*SubCategory, init func(*SubCategory), assign func(*SubCategory, *Product)) error {
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subcategory.ProductsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_subcategory
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_subcategory" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_subcategory" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (scq *SubCategoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(subcategory.Table, subcategory.Columns, sqlgraph.NewFieldSpec(subcategory.FieldID, field.TypeUUID))
	_spec.From = scq.sql
