...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withSubcategory; query != nil {
		if err := pq.loadSubcategory(ctx, query, nodes, nil,
			func(n *Product, e *SubCategory) { n.Edges.Subcategory = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadSubcategory(ctx context.Context, query *SubCategoryQuery, nodes []*Product, init func(*Product), assign func(*Product, *SubCategory)) error {
	...
	for i := range nodes {
		if nodes[i].product_subcategory == nil {
			continue
		}
		fk := *nodes[i].product_subcategory
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_subcategory" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
